...
<?php
    ...
    $sql = "SELECT COUNT(*) AS total_lecturer FROM researches";
    $result = $conn->query($sql);
    $row = $result->fetch_assoc();
    $totalLecturer = $row['total_lecturer'];

    $sql = "SELECT COUNT(*) AS total_expert FROM experts";
    $result = $conn->query($sql);
    $row = $result->fetch_assoc();
    $totalExpert = $row['total_expert'];
...
<script>
    ...
    // Define the data
    var data = {
        labels: ['Total User', 'Total Lecturer', 'Total Experts'],
        datasets: [{
            label: 'Total Count',
            data: [<?php echo $totalUser; ?>, <?php echo $totalLecturer; ?>, <?php echo $totalExpert; ?>],
            backgroundColor: [
